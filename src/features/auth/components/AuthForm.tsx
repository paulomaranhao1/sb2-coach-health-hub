...
const AuthForm = () => {
  ...
  if (mode === 'email-verification') {
    return <EmailVerificationScreen email={emailForVerification} onBackToLogin={() => setMode('signin')} />;
  }
  if (mode === 'forgot-password') {
    return <Card className="w-full max-w-md mx-auto">
        <CardContent className="pt-6">
          <AuthHeader isLogin={true} />
          <ForgotPasswordForm onSubmit={handleForgotPassword} onBackToLogin={() => setMode('signin')} loading={loading} />
        </CardContent>
      </Card>;
  }
  if (mode === 'magic-link') {
    return <Card className="w-full max-w-md mx-auto">
        <CardContent className="pt-6">
          <AuthHeader isLogin={true} />
          <MagicLinkForm onSubmit={handleMagicLink} onBackToLogin={() => setMode('signin')} loading={loading} />
        </CardContent>
      </Card>;
  }
  return <Card className="w-full max-w-md mx-auto">
      <CardContent className="pt-6 py-[17px]">
        ...
        <div className="space-y-4">
          ...
          <EmailPasswordForm isLogin={mode === 'signin'} onSubmit={handleEmailFormSubmit} loading={loading} />
        </div>
      </CardContent>
    </Card>;
};
