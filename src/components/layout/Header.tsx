...
const Header = ({
  ...
  return <>
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              {!hasPremiumAccess && <AnimatedButton onClick={handlePurchase} size="sm" className="hidden sm:flex shadow-button hover:shadow-button-hover">
                  <ShoppingCart className="w-4 h-4 mr-2" />
                  Comprar SB2 Turbo
                </AnimatedButton>}
              
              {subscription?.verification_status === 'pending' && <Badge variant="secondary" className="hidden sm:flex animate-pulse bg-slate-100 text-slate-700">
                  Verificando...
                </Badge>}
              
              {remindersEnabled && pendingReminders > 0 && <Button onClick={handleRemindersClick} size="sm" variant="outline" className={`hidden sm:flex shadow-md hover:shadow-lg transition-all duration-200 ${visualAlertEnabled ? 'animate-bounce-subtle' : ''}`}>
                  <Bell className="w-3 h-3 mr-1" />
                  {pendingReminders} lembrete{pendingReminders > 1 ? 's' : ''}
                </Button>}
              <Button size="sm" variant="outline" className="shadow-md hover:shadow-lg hover:scale-105 transition-all duration-200" onClick={() => setShowMobileMenu(!showMobileMenu)}>
      ...
      <RemindersModal open={showRemindersModal} onOpenChange={setShowRemindersModal} onNavigateToSupplements={handleNavigateToSupplements} />
    </>;
};
